...
OpenClaw 的 AI Agent 可以通过**消息工具（Messaging Tool）**主动向用户或其他渠道发送消息——例如使用 `message` 工具向 Discord 频道发送一条通知。问题在于：模型在使用 `message(action="send", to="user", content="Hello!")` 工具后，往往还会在其最终文本回复中重复同样的内容（"Hello!"）。如果不做去重，用户会收到**两条相同的消息**：一条来自工具调用，一条来自最终回复。
