...
在前面的章节中，我们看到每个会话是一个独立的对话容器。但在实际使用中，Agent 常常需要与其他会话中的 Agent 协作——比如主 Agent 将编码任务委托给一个子代理（Sub-agent），或者查询另一个会话的历史记录。OpenClaw 通过一组**会话工具**（`sessions_list`、`sessions_history`、`sessions_send`）实现了这种跨会话通信能力。
