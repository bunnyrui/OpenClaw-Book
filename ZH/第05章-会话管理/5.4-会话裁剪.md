...
在 Agent 的工作过程中，工具调用的结果（如 bash 命令的输出、文件内容、搜索结果等）往往占据上下文的大量空间。OpenClaw 在 `src/agents/pi-extensions/context-pruning/` 目录下实现了一套两阶段裁剪策略：**软裁剪**（Soft Trim）和**硬清除**（Hard Clear）。
