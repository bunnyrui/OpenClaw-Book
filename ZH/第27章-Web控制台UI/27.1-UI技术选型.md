...
值得注意的是，`OpenClawApp` 类没有覆盖 `createRenderRoot()` 方法来启用 Shadow DOM——这意味着整个控制台 UI 的样式是全局的（通过 `ui/src/styles.css` 和 `ui/src/styles/` 目录定义），而非组件级隔离的。这是一个实用性选择：管理界面不需要样式隔离的严格封装，而全局样式更便于主题切换和统一调整。
