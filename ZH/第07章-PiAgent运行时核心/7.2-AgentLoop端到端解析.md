...
当一条用户消息通过通道和路由最终需要 AI 回复时，它会触发一次完整的 **Agent Loop**（Agent 循环）。这个循环是 OpenClaw 中最复杂的流程之一，涉及参数校验、模型解析、凭据准备、队列序列化、LLM 调用、错误恢复等多个阶段。本节将端到端地跟踪一条消息从进入 Gateway 到产生 AI 回复的完整路径。
