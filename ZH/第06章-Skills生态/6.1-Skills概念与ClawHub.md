...
## 6.1.2 Skills 的类型：Bundled / Managed / Workspace

根据来源和作用域，OpenClaw 里常见 Skill 可以分成三类（本章按实战视角使用这三类）：

### 1）Bundled Skills（内置技能）

- 随 OpenClaw 一起分发；
- 开箱就有，适合通用场景；
- 例如：`coding-agent`、`github`、`weather`、`canvas` 等。
...
- 常见 Skill 来源有 bundled / managed / workspace，优先级是 `workspace > managed > bundled`；
