...
前两节我们分别解析了 exec 工具的命令执行引擎和 process 工具的进程管理机制。本节将聚焦于支撑它们运行的**共享基础设施**：Shell 检测与配置、Docker 沙箱环境构建、工作目录解析、二进制输出清理、以及进程树管理。这些看似简单的工具函数，实则隐藏着大量的跨平台兼容性考量。
