...
OpenClaw 的 iOS 应用与 macOS 应用有本质区别：macOS 应用是一个**控制端**（operator），而 iOS 应用是一个**节点**（node）。节点不运行 Gateway，而是连接到局域网中的 Gateway 实例，将 iPhone/iPad 的硬件能力（摄像头、屏幕、麦克风、位置）暴露给 AI 代理使用。
...
iOS 应用的主界面是一个 **Canvas 表面**——它不仅是聊天界面，更是 AI 代理可以直接操控的"画布"。代理可以发送 Canvas 命令来控制界面内容，实现诸如展示图表、动画、交互式控件等能力。
...
iOS 节点的独特价值在于将移动设备的传感器暴露给 AI 代理：

**摄像头**（`Camera/`）：代理可以请求拍照或实时视频帧，用于视觉分析。摄像头能力通过 `AVCaptureSession` 实现。

**屏幕录制**（`Screen/`）：代理可以请求屏幕截图或录制。iOS 使用 `ReplayKit` 框架提供屏幕内容捕获。
...
Gateway 根据节点声明的能力来决定哪些工具可以使用——例如，只有连接了具备 `camera` 能力的节点时，代理才能调用拍照工具。
...
3. **Canvas 表面**是主界面，AI 代理可以通过命令直接操控界面内容。
